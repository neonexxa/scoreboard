...
<script src="js/form_chal.js "></script>
			...
			$queryround = "select title from game_round where round_ref='F'";
			echo "<br/>";
			
			$teamref = getteamref($mysqli,$_SESSION['team_name']);
			$sth = $mysqli->prepare("SELECT soalan_ref FROM F_solved where team_ref = '$teamref'");
			$sth->execute();
			...
			while ($stmt->fetch()) {
				...
				if (in_array($assigned_to, $question_solved) && ($statuschal == "")) {
					...
					echo "<span id='head1'>Challenge " . ++$nochal;
					if ($nochal == 10) { echo " - Bonus";}
					echo "</span><span style='padding-left:115px'id='status_chal" . $nochal . "'>$statuschal</span><br/>" . $endingtag;
				if ($statuschal == "") {
