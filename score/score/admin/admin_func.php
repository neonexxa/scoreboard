    ...
    if (isset($_SESSION['username'], 
        ...
        if ($stmt = $mysqli->prepare("SELECT pwd 
            ...
            if ($stmt->num_rows == 1) {
                ...
                $stmt->fetch();
                $login_check = hash('sha512', $password . $user_browser) . "aaaaadddddmmmiiinnnnnn";
...
function login_admin($username, $password, $mysqli) {
$now = time();
$client_ip = get_client_ip();
    if ($stmt = $mysqli->prepare("SELECT id, username, pwd, active 
        FROM admin_cred
       WHERE username = ?
        LIMIT 1")) {
        ...
        if ($stmt->num_rows == 1) {  //if there is any result from query and the user is active
                if (($db_password == $password) && ($active)) { //if passwd from form same as from db and user is active
					...
					$_SESSION['login_string'] = hash('sha512', $password . $user_browser) . "aaaaadddddmmmiiinnnnnn";
					echo hash('sha512', $password . $user_browser) . "aaaaadddddmmmiiinnnnnn";
					$mysqli->query("INSERT INTO admin_login_log(username, ip, time) VALUES ('$username',' $client_ip', '$now')");
                    return true;
                } else {
                    return false;
                }
