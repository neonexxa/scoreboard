...
<script src="js/form_chal.js "></script>
			...
			echo $ip;
			$queryround = "select title from game_round where round_ref='G1'";
			...
			echo "<br/>";
			//$decc = encryptt("hai", KEYY);
			//echo $decrypted = decryptt($decc, KEYY);
			
			$teamref = getteamref($mysqli,$_SESSION['team_name']);
			
			$sth = $mysqli->prepare("SELECT soalan_ref FROM G1_solved where team_ref = '$teamref'");
			$sth->execute();
			...
			//print_r($question_solved);
			
			//$question_solved = implode(",", array_map(function($x) { return "'$x'"; }, $question_solved));
			
			
//			$sql = "SELECT question, assigned_to FROM bank_soalan WHERE assigned_to LIKE ? AND assigned_to NOT IN ('G1Q1','G1Q2','G1Q3')";
			...
			while ($stmt->fetch()) {
				...
				if (in_array($assigned_to, $question_solved) && ($statuschal == "")) {
					$statuschal = "Solved <img src='./images/green_tick.png' title='Solved'/>";
					$endingtag = "<br/>";
				}
