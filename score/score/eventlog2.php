...
echo "<br/>";
	...
	$teamref = "C3";
	$teamname = getteamname($mysqli,$teamref);
	...
	//$sql = "SELECT * FROM team_login_success union all SELECT * FROM team_login_attempts union all SELECT * FROM " . $current_round . "_attempts union all SELECT * FROM " . $current_round . "_solved ORDER BY 4 desc";
	$sql = "SELECT * FROM " . $current_round . "_attempts union all SELECT * FROM " . $current_round . "_solved ORDER BY 4 desc";	
	$stmt = $mysqli->stmt_init();
			...
			while ($stmt->fetch()) {
				...
				if ($lima == "LA")
					...
					echo "Successfully login from $dua";
				else if ($lima == "GA")
					echo "Incorrect flag submitted - Challenge " . substr($dua,-1) . "<b> : " . base64_decode($enam) . "</b>";
				if ($lima == "GS")
					echo "Correct flag submitted - Challenge " . substr($dua,-1) . "<b> : " . base64_decode($enam) . "</b>";
				echo "<br/>";
				}
			}
			$stmt->close(); 
